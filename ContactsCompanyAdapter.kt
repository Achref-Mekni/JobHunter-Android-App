...
class ContactsCompanyAdapter(private val context: Context, list: ArrayList<User>) : RecyclerView.Adapter<ContactsCompanyAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.name_user.text = user.name + " " + user.last_name


        if (user.picture!!.length > 60) {
            Picasso.with(context as Main2Activity).load(user.picture).into(holder.img_user)
        } else {
            Picasso.with(context as Main2Activity).load(GlobalParams.ressourceUrl + "/" + user.picture).into(holder.img_user)
        }
